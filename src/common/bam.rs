...
pub fn build_chroms_bam(
    ...
    regex: Option<String>,
) -> Result<Vec<Interval>, anyhow::Error> {
    let mut result: Vec<Interval> = Vec::new();

    let re = regex.map(|regex| Regex::new(&regex).unwrap());
...
pub fn samples_from_file<P: AsRef<Path>>(path: P) -> Result<Vec<String>, anyhow::Error> {
