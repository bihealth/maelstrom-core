...
impl ReferenceStats {
    ...
    pub fn from_path<P: AsRef<Path>>(
        ...
        match path.as_ref().to_str() {
            ...
            _ => Err(anyhow::Error::new(AppError::InvalidPath)),
        }
    }

    /// Internal builder function.
    fn build(path: &str, chrom: &str, window_length: usize) -> Result<Self, anyhow::Error> {
        Term::stderr().write_line("Loading GC content and gap (is-N) status...")?;
    ...
    fn load_seq(path: &str, chrom: &str) -> Result<Text, anyhow::Error> {
        Term::stderr().write_line(&format!(
            "Loading reference sequence {}: {}...",
            path, chrom
        ))?;

        Term::stderr().write_line("Opening indexed FASTA reader")?;
        let mut ref_reader = fasta::IndexedReader::from_file(&path)?;

        Term::stderr().write_line(&format!("Reading reference {}", chrom))?;
        let mut seq = Text::new();
        ref_reader.fetch_all(chrom)?;
        ref_reader.read(&mut seq)?;
        let end = seq.len();
        Term::stderr().write_line(&format!("Reference seq {} has {} characters", chrom, end))?;
    ...
    fn look_at_chars(
        seq: &[u8],
        window_length: usize,
    ) -> Result<(Vec<f32>, Vec<bool>), anyhow::Error> {
        Term::stderr().write_line("Analyzing sequence composition...")?;

        let num_buckets = ((seq.len() + window_length - 1) / window_length) as usize;

        // Count, GC characters, non-N chracters, and record "is N" flags.
        let mut gc_count = vec![0_i32; num_buckets];
        let mut non_n_count = vec![0_i32; num_buckets];
        let mut has_gap = vec![false; num_buckets];

        // Count GC chars and establish gap status.
        Term::stderr().write_line("Counting GC and N characters...")?;
        ...
        Term::stderr().write_line("Computing GC content from GC counts...")?;
